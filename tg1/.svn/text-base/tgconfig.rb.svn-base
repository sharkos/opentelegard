...
:path_sqldumps, :path_msgsareas, :recent_min, :recent_max,
		...
		if makeconfig.upcase == "Y"
		   ...
		   puts "Creating default configuration file..."
			   ...
			   cfgout.revision		= @revision
				...
				cfgout.recent_max			= @recent_max
				cfgout.recent_min			= @recent_min
...
def ncadminbbs(cfgin)
	...
	while submenuloop > 0
	   ...
	   submenu.mvhline(12,1,0,76)
		...
		submenu.mvaddstr(7,2, "6. Show last # recent users:")
		submenu.mvaddstr(8,2, "7. Maximum # in Recent File:")
	   ...
	   submenu.attroff(Ncurses.COLOR_PAIR(6))
    	...
    	submenu.attron(Ncurses.COLOR_PAIR(3))
	 	...
	 	submenu.mvaddstr(7, 31, "#{cfgin.recent_min}")
	 	submenu.mvaddstr(8, 31, "#{cfgin.recent_max}")
#	 	submenu.mvaddstr(9, 31, "#{cfgin.db_socketpath}")
	   ...
	   case submenuitem
		...
		when '6'[0] # Maximum number to display output of recent file
			 clearline = " " * 2
			 submenu.mvaddstr(7, 31, "#{clearline}")
			 submenu.move(7,31)
	  		 submenu.getstr(newvalue)
				unless newvalue.empty?
					cfgin.recent_min = newvalue.to_i
				end

		when '7'[0] # Maximum number to store in recent file before archiving to reporting DB
			 clearline = " " * 2
			 submenu.mvaddstr(8, 31, "#{clearline}")
			 submenu.move(8,31)
	  		 submenu.getstr(newvalue)
				unless newvalue.empty?
					cfgin.recent_max = newvalue.to_i
				end
